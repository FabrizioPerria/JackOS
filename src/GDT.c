...
* granularity                                                           *
*************************************************************************/
void gdt_setEntry(int numIndex,
				unsigned long base,
				unsigned long limit,
				unsigned char access,
				unsigned char granularity)
...
void gdt_install()
{
/* Define the table; this pointer will be given directly to the GDT register
   when the table is filled up */
	gdt_ptr.limit = (sizeof(struct gdtEntry) * 3) - 1;
	gdt_ptr.base = (unsigned int)&gdtTable;

/*  Usually we gotta create 3 entries; the first will be NULL, the second is
    going to be the code descriptor and the last one will contain the data
    descriptor */
...
/* This function is written in assembly language because we gotta use the
   lgdt function to load the table in the GDT register */
	gdt_flush();
}
