...
static int getNextClusterFromFAT(int drive, int index) {
  ...
  next = _mi[drive].fat[FAT_offset] + (_mi[drive].fat[FAT_offset + 1] * 0x100);
  if (index % 2)
...
static void writeOnFAT(int drive, int index, int value) {
  ...
  writeLBA28(drive, _mi[drive].fatPosition + _mi[drive].fatSize, 9,
             _mi[drive].fat);
}
...
FILE FAT12Create(char *fileName) {
  FILE folder, file;
  FILE_PTR list;
  int i, j = 1, tmp = 0, len = 0, lastCluster, freeCluster, nextEntry = 0,
         nextCluster = 0, pos = 0, phySector = 0, drive = -1;
  ...
  if (pos > 0 && (pos % 16 == 0)) {
    ...
    lastCluster = findLastCluster(drive, folder.currentCluster * 1.5);
  ...
  */
  directory->time =
      ((time[2] & 0x1F) << 11) | ((time[1] & 0x3F) << 5) | (time[0] & 0x1F);
  directory->date = (((time[5] - 80) & 0x5F) << 9) | ((time[4] & 0xF) << 5) |
                    (time[3] & 0x1F);
  /* Write the record */
...
FILE *FAT12List(FILE folder) {
  ...
  if (folder.flags == FS_DIRECTORY) {
    for (j = 0; j < (int)folder.length; j++) {
      ...
      directory +=
          (j % (_mi[folder.deviceID].sectorSize / sizeof(struct fat12Entry)));
      ...
      if (directory->attribute == 0x10) {
        ...
        while (1) {
          nextCluster =
              getNextClusterFromFAT(folder.deviceID, tmp.currentCluster);
    ...
    memcpy((unsigned char *)&chain[0], (unsigned char *)&folder,
           sizeof(folder));
    return chain;
  } else {
...
FILE FAT12Directory(int drive, char *name, FILE *folder) {
  ...
  while (1) {
    ...
    for (j = 0; j < (int)(_mi[drive].sectorSize / sizeof(struct fat12Entry));
         j++) {
...
int FAT12Mount(int drive) {
  ...
  _mi[drive].rootPosition =
      (bootsector->numberFATs * bootsector->sectorsPerFAT) + 1;

  _mi[drive].rootSize =
      (bootsector->maxNumberEntriesRoot * 32) / bootsector->bytesPerSector;
