...
int phy_manager_init(int memory_size, int *bitmapAddr) {
  ...
  memset((unsigned char *)bitmapPtr, 0xff,
         (maxBlocks / PHY_MANAGER_BLOCKS_PER_BYTE));

  return 0;
}

/* Set a block as used in the bitmap */
inline static void mmap_set(int bit) {
  if (bit > 0)
    bitmapPtr[bit / 32] |= (1 << (bit % 32));
}

/* Set a block as free in the bitmap */
inline static void mmap_unset(int bit) {
  if (bit > 0)
    bitmapPtr[bit / 32] &= ~(1 << (bit % 32));
}

inline static int mmap_test(int bit) {
  if (bit > 0)
...
/* Initialize a region described by its base address and its length (info passed
by the BIOS through multiboot structure) */
...
void PHYinit(struct multiboot_info *mbootPtr, int kernelSize) {
  ...
  struct multiboot_mmap_entry *region =
      (struct multiboot_mmap_entry *)mbootPtr->mmapAddress;
  /* Initialize the entire memory
          the memory size is given by the multiboot structure (so is provided by
     the BIOS at startup) |   BIOS   |
          ----------- 0x100000
          |  KERNEL  |
          |          |
          ----------- 0x100000 + kernelSize (multiplied by 512 because is given
     in number of clusters) |          | |   ...    | MEMORY AVAILABLE | |
  */

  phy_manager_init(getMemorySize(mbootPtr),
                   (int *)(0x100000 + (kernelSize * 512)));
  ...
  for (i = 0; i < 10; i++) {
    ...
    print("region %d: start: 0x%x length: %x end: %x type: %d\r\n", i,
          (region[i].addressLow), (region[i].lengthLow),
          (region[i].addressLow) + (region[i].lengthLow) - 1, region[i].type);
    if (region[i].type == 1) {
