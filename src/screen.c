...
static unsigned short *textPtr =
    (unsigned short *)(VGA_MEMORY + ((VGA_WIDTH) * 10 * 2));
...
void scrollEnable() { scrollIt = 1; }

void scrollDisable() { scrollIt = 0; }
...
void scrollDown() {
  if (yCursor >= 25) {
    ...
    memsetWord((unsigned short *)textPtr + ((VGA_WIDTH) * (VGA_HEIGHT - 1)),
               (color << 8 | ' '), VGA_WIDTH);
    xCursor = 0;
    yCursor--;
  }
...
void refreshTimer() {
  ...
  if (!fullscreen) {
    ...
    asm("cli");
    moveCursor(0, 9);
    print("%2x-%2x-%2x%2x %2x:%2x:%2x\r\n", timeDate[3], timeDate[4],
          timeDate[6], timeDate[5], timeDate[2], timeDate[1], timeDate[0]);
    moveCursor(tmpX, tmpY);
    asm("sti");
  }
}

int getXCursor() { return xCursor; }

int getYCursor() { return yCursor; }
